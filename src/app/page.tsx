      ...
      <main className="flex-1 container mx-auto">
        ...
        <div className="flex flex-wrap nav-bar gap-2 sm:gap-3 mb-6 sm:mb-8 mt-6">
          {allCategories.map((category, index) => (
            <button
              key={index}
              className={`px-4 py-2 rounded-button whitespace-nowrap transition-all text-sm font-medium ${activeCategory === category.code
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          {filteredTools().map((tool, index) => (
            <div
              key={index}
              className="card p-6 flex flex-col cursor-pointer min-h-[160px]"
